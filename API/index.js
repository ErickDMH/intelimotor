...
async function fillFormFirstSegment(page) {
	await timeout(1000)
	...
	await page.type('#input_precio', '96000');
	console.log('Uploaded first segment of form.');
}
async function fillFormSecondSegment(page) {
	await page.waitForSelector('#input_text_area_review');
	await page.type('#input_text_area_review', 'ejemplo headless demo');
	await timeout(3000)
...
(async () => {
	...
	await fillFormFirstSegment(page)
	await page.click('button[class="next-button"]');
	await fillFormSecondSegment(page)
